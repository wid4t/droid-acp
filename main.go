...
func handleACPRequest(req types.ACPRequest) {
	...
	case "session/new":
		...
		// Initialize droid session with the provided cwd
		cwd := params.Cwd
		if cwd == "" {
			cwd = "."
		}
...
func handleDroidMessage(msg types.DroidMessage) {
	switch msg.Method {
	case "droid.initialize_session", "droid.update_session_settings":
		sendDroidOK(msg.ID)
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if msg.Method != "" {
				...
				fmt.Fprintf(os.Stderr, "[DROID RESPONSE] id=%s\n", msg.ID)
			}
		}
	}()
